...
| 4 Jul 2026 | **Per-instrument auto-execute** | Added `autoExecuteEnabled` boolean to `Instrument` entity + `WatchlistProperties.InstrumentConfig`. Synced from YAML to DB on restart (`DataInitializer`). SOL/BTC/ETH set `auto-execute-enabled: true` — skip the 120s Telegram confirmation keyboard and place IG demo deals immediately. S&P/DAX/Gold/Silver keep manual approval. `IGTradingService.handleSignalEvent` checks the flag before sending the confirmation keyboard. See `AGENTS.md#2026-07-04`. |
| 4 Jul 2026 | **Quiet-hours bypass for auto-execute instruments** | `IGTradingService.isQuietHours()` gate now skipped when the instrument has `autoExecuteEnabled=true`. Manual-approval instruments (S&P/DAX/Gold/Silver) still respect quiet hours so their confirmation keyboard never fires at 3am. `NotificationService` suppresses Telegram pushes for ALL instruments during quiet hours — overnight auto-executed trades are silent and appear in the next `/pnl-report`, same as the existing `trend-buy-dip-notify:false` silent pattern. `PositionOutcomeService` mirrors the auto-execute exemption (Jul 12 fix: was unconditionally skipping all position opens during quiet hours, causing orphaned IG trades). |
| 12 Jul 2026 | **Orphaned overnight crypto auto-trades fix** | `PositionOutcomeService.handleSignalEvent` was unconditionally returning during quiet hours, while `IGTradingService` placed live IG deals for auto-execute instruments — resulting in orphaned trades (live on IG, no DB tracking, no trailing, invisible in reports). Fixed by mirroring the auto-execute exemption: auto-execute instruments now open a tracked DB position during quiet hours. Also upgraded `handleConfirm`'s orphan fallback from `log.warn` to `log.error` + Telegram alert for immediate visibility. |
| 4 Jul 2026 | **AdxCalculator bounded query** | `computeAdx` was using the unbounded `findBySymbolAndTimeframeOrderByCandleTimeAsc` — the last remaining full-table-per-symbol scan on the hot path (called every poll cycle per instrument via `TrendDetectionService`). Now uses `findBySymbolAndTimeframeOrderByCandleTimeDesc(..., PageRequest.of(0, n))` + reverse, same bounded pattern as `AtrCalculator` (May 2026). Fetch size: `period * (2 + 5) + 1` = 99 candles for period=14. Removed the unbounded repository method entirely to prevent accidental reuse. |
